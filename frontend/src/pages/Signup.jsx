...
import { UserPlus, Mail, Lock, RefreshCw } from 'lucide-react';
...
export default function Signup() {
  ...
  const handleEmailSignUp = async (e) => {
    ...
    if (password.length < 6) {
      setError('Access passcode must be at least 6 characters');
      return;
    }
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') {
        setError('Operator identity exists. Email already registered.');
      } else if (err.code === 'auth/weak-password') {
        setError('Operator passcode is too weak.');
      } else {
        setError(err.message || 'Registration failed. Check system constraints.');
      }
    } finally {
      setLoading(false);
    }
