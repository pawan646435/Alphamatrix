...
Methodology:
  ...
  - Compares against a flat 12% annualized NIFTY 50 proxy (bench_30/90/180/365
    below are a straight-line 12%/year rate, NOT an actual blended Reliance +
    HDFCBANK — or any other real index/basket — return series; no such
...
INDEPENDENCE FROM LIVE VERDICTS: `historical_verdict` below is generated by
`_composite_score`/`_score_to_verdict` — a separate, price-history-only
model (momentum/CAGR/drawdown, thresholds 75/62/47/33) that is completely
independent of both the Alpha Score model (workers/stock_ingestion.py:
calculate_institutional_ratings) and the AI briefing rubric
(services/briefing_intelligence.py:score_verdict). `current_verdict`/
`current_score` are accepted by backtest_stock() purely for display
alongside the historical result — they are never read by the success/failure
calculation, so this module cannot "invalidate" or confirm today's live
verdict; it only reports how its own separate historical model would have
performed. See the frontend's "Independent Technical Backtest" disclaimer
(StockDetail.jsx / StockHome.jsx) for how this is surfaced to users.
"""
