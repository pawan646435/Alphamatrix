...
async def run_overnight_sync() -> dict:
    """
    ...
    Opens its own DB session rather than accepting one from the caller: this
    function is only ever invoked detached from a request (BackgroundTasks
    fallback in api/v1/funds.py:/sync-all), and a request-scoped session would
    already be closed by the time a background task actually runs (Starlette
    tears down request dependencies before executing background tasks).
    """
